...
public class GraveClusterBlock : PropsBlock
{
    ...
    private void TryAddGraveDirt(GraveYardLayoutProfile graveYardLayoutProfile, GameObject graveGO)
    {
        if (Random.Range(1, 7) > 4)
        {
            graveGO.transform.GetChild(0).gameObject.SetActive(true);
            graveGO.transform.GetChild(0).GetComponent<SpriteRenderer>().sprite = graveYardLayoutProfile.graveDirtSprites[Random.Range(0, graveYardLayoutProfile.graveDirtSprites.Length)];
        }
    }

...
public class BlazingArcHandler : ActiveAbilityHandler
{
    ...
    private IEnumerator SpawnBlazingArcCoroutine()
    {
        while (true)
        {
            ...
            {
                Vector3 spawnPos = player.transform.position + new Vector3 (playerController.PlayerMovementHandler.FaceDirection.x, player.GetComponent<SpriteRenderer>().bounds.size.y/2,0); 
